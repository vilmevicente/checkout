        ...
        function checkout() {
            return {
                ...
                init() {
                    ...
                    @if(old('name'))
                    this.customer.name = '{{ old('
                    name ') }}';
                    this.fields.name = true;
                    @endif
                    @if(old('email'))
                    this.customer.email = '{{ old('
                    email ') }}';
                    this.fields.email = true;
                    @endif
                    @if(old('email_confirmation'))
                    this.customer.email_confirmation = '{{ old('
                    email_confirmation ') }}';
                    this.fields.email_confirmation = true;
                    @endif
                    @if(old('phone'))
                    this.customer.phone = '{{ old('
                    phone ') }}';
                ...
                validateField(field) {
                    ...
                    switch (field) {
                        ...
                        case 'phone':
                            ...
                            } else if (!this.isValidDDD(this.customer.phone)) {
                                this.errors.phone = 'DDD inválido.';
                            } else {
                                this.errors.phone = '';
                            }
                            break;
...
formData.append('phone', cleanPhone);
                        ...
                        if (data.success) {
                            ...
                            this.customer.name = '';
                            this.customer.email = '';
                            this.customer.email_confirmation = '';
                            this.customer.phone = '';
                            ...
                            QRCode.toCanvas(canvas, this.pixCode, {
                                width: 200
                            }, function(error) {
                                if (error) {
                                    console.error(error);
                ...
                // Adicione esta função no seu objeto checkout()
                resetForm() {
                    this.customer = {
                        name: '',
                        email: '',
