...
<div x-data="productForm()">
    ...
    <form @submit.prevent="submitForm" class="bg-white rounded-lg shadow-md p-6 mb-6">
        @csrf
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">URL do Banner Principal</label>
                    <input type="url" name="main_banner" x-model="formData.main_banner"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">URL do Banner Secundário</label>
                    <input type="url" name="secondary_banner" x-model="formData.secondary_banner"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
                </div>
            </div>
        </div>
        ...
        <div class="mb-8">
            <h2 class="text-xl font-semibold text-gray-800 mb-4 border-b pb-2">
                <i class="fas fa-tags mr-2"></i> Upsells
            </h2>
                    ...
                    <div class="upsell-item p-4 border border-gray-200 rounded-lg bg-gray-50">
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                            <div class="md:col-span-5">
                                ...
                                <select :name="`upsells[${index}][product_id]`" x-model="upsell.product_id" required
                                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
                    ...
                    <div class="attachment-item p-4 border border-gray-200 rounded-lg bg-gray-50">
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                            <div class="md:col-span-5">
                                ...
                                <input type="text" :name="`attachments[${index}][name]`" x-model="attachment.name" required
                                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
                            </div>
                            <div class="md:col-span-5">
                                <label class="block text-sm font-medium text-gray-700 mb-2">URL do Arquivo</label>
                                <input type="url" :name="`attachments[${index}][file_url]`" x-model="attachment.file_url" required
                                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
                            </div>
...
function productForm() {
    return {
        formData: {
            ...
            original_price: '{{ old("original_price", $product->original_price ?? "") }}',
            description: `{{ old("description", $product->description ?? "") }}`,
            main_banner: '{{ old("main_banner", $product->main_banner ?? "") }}',
            secondary_banner: '{{ old("secondary_banner", $product->secondary_banner ?? "") }}',
            delivery_content: `{{ old("delivery_content", $product->delivery_content ?? "") }}`,
            is_active: {{ old("is_active", isset($product) ? $product->is_active : "true") }},
            upsells: @json(old('upsells', isset($currentUpsells) ? $currentUpsells->map(function($discount, $id) {
                return ['product_id' => $id, 'discount_price' => $discount['discount_price']];
            })->values() : [])),
            attachments: @json(old('attachments', isset($product) ? $product->attachments->map(function($attachment) {
                return [
                    'id' => $attachment->id,
            ...
            delivery_methods: @json(old('delivery_methods', isset($product) ? $product->deliveryMethods->pluck('id')->toArray() : []))
        },
        ...
        removeUpsell(index) {
            if (this.formData.upsells.length > 1) {
                this.formData.upsells.splice(index, 1);
            } else {
                alert('É necessário ter pelo menos um upsell.');
            }
        },
        ...
        removeAttachment(index) {
            if (this.formData.attachments.length > 1) {
                ...
                alert('É necessário ter pelo menos um anexo.');
            }
        },
        ...
        submitForm() {
            ...
            const productIds = this.formData.upsells.map(upsell => upsell.product_id);
