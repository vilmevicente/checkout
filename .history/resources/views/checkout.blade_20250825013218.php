...
<body>
                ...
                <div class="flex items-center gap-4 text-sm text-gray-600 mt-4 md:mt-0">
                    <div class="flex items-center gap-2">
                        <svg class="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 极速赛车开奖直播 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path>
                        </svg>
    ...
    <div class="container px-4 py-8 flex-1">
        <!-- Banner principal -->
        <img src="/{{ $mainProduct['mainbanner'] }}" alt="Banner Principal" class="w-full mb-8 rounded-lg shadow-card">
            ...
            @foreach ($mainProduct['features'] as $feature)
                <div class="benefit-card shadow-card">
                    <div class="icon-container">
                        <i class="benefit-icon {{ $feature['icon'] }}"></i>
                    </div>
        ...
        <div class="grid lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-elevated p-6 mb-8">
                    ...
                    <div class="flex items-start gap-4 border rounded-xl p-4 mb-6 upsell-item">
                        <img src="{{ $upsell['image'] }}" alt="{{ $upsell['name'] }}" class="w-16 h-16 rounded-lg object-cover">
                        <div class="flex-1">
                            <label for="upsell-{{ $upsell['id'] }}" class="font-medium text-sm cursor-pointer block">
                                ...
                                {{ $upsell['name'] }} - <span class="font-semibold">R$ {{ number_format($upsell['price'], 2, ',', '.') }}</span>
                            </label>
                            <p class="text-xs text-gray-500 mt-1">De <span class="line-through">R$ {{ number_format($upsell['originalPrice'], 2, ',', '.') }}</span> por apenas {{ $upsell['discount'] }}</p>
                        </div>
                    </div>
                    @endforeach
                </div>
                ...
                <div class="bg-white rounded-xl shadow-card p-6">
                    ...
                    <div class="flex justify-between items-center text-sm mb-4">
                        ...
                        <span id="savings-amount" class="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded">
                            R$ {{ number_format($mainProduct['originalPrice'] - $mainProduct['price'], 2, ',', '.') }}
                        </span>
                    </div>
                    ...
                    <div class="flex justify-between items-center text-lg font-bold pt-2 border-t border-gray-200">
                        <span>Total:</span>
                        <span id="total-amount" class="text-indigo-600">R$ {{ number_format($mainProduct['price'], 2, ',', '.') }}</span>
                    </div>
            ...
            <div class="space-y-6">
                <!-- Secondary Banner (condicional) -->
                @if(isset($mainProduct['secondarybanner']) && $mainProduct['secondarybanner'])
                <img src="{{ $mainProduct['secondarybanner'] }}" alt="Banner Secundário" class="w-full mb-4 rounded-lg shadow-card">
                @endif
    ...
    <script>
        ...
        document.getElementById('pay-button').addEventListener('click', function() {
            ...
            qrElement.innerHTML = '';
            QRCode.toCanvas(qrElement, pixCode, { width: 200 }, function(error) {
