...
@foreach($mainProduct->upsells as $index => $upsell)
    @php
        $discountPrice = (float) $upsell->pivot->discount_price;
    ...
    <div class="flex items-start gap-4 border rounded-xl p-4 mb-6 upsell-item"> 
        <img src="{{ $upsell->main_banner ?? 'https://via.placeholder.com/64' }}" 
             alt="{{ $upsell->name }}" 
        ...
        <div class="flex-1">
            <label for="upsell-up{{ $index }}" class="font-medium text-sm cursor-pointer block">
                ...
                {{ $upsell->name }} - 
                <span class="font-semibold">R$ {{ number_format($discountPrice, 2, ',', '.') }}</span>
            </label>

            <p class="text-xs text-gray-500 mt-1">
                De <span class="line-through">R$ {{ number_format($originalPrice, 2, ',', '.') }}</span> 
                por apenas {{ $discountPercent }}% OFF
            </p>
        </div>
    </div>
@endforeach
