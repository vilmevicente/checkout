...
class PagFanController extends Controller
{
    ...
    private function processTransaction($transaction)
    {
        ...
        if ($order && $transaction['status'] === 'RECEIVEPIX') {
            $order->markAsPaidupdate($transaction['transactionId']);
            
            

               try {
            // Enviar e-mail para o produto principal
            $this->sendProductEmail($order, $order->products);
            
            // Enviar e-mails para cada upsell
...
private function sendProductEmail($order, $product)
{
    try {
        ...
        \Mail::send('admin.emails.product-delivery', [
            ...
            if ($product->delivery_file && Storage::exists($product->delivery_file)) {
                $message->attach(
                    Storage::path($product->delivery_file),
                    ['as' => $product->name . '.pdf']
                );
            }
        });
        
        \Log::info('E-mail enviado para: ' . $product->name);
